...
const AvCreation = ({ role, email }) => {
  return (
    <ThemeProvider>
      <Article role={role} email={email} />
    </ThemeProvider>
  );
};

const Article = ({ role, email }) => {
  ...
  const handleDelete = async (articleId) => {
    try {
      ...
      const blocs = blockResponse.data['hydra:member'].filter(
        (block) => block.Idarticle === '' + articleId
      );
